...
class GlobalSuperpowersWorkflow:
    ...
    def brainstorming(self, requirement: str, output_file: Optional[str] = None) -> str:
        ...
        if output_file is None:
            output_file_path = self.output_locations['design_docs'] / f"{phase}_{timestamp}.json"
        else:
        ...
        # 创建对应的 Markdown 文件用于审阅
        markdown_file = output_file_path.with_suffix('.md')
        self._create_brainstorming_markdown(design_doc, markdown_file)
        
        self._log(phase, f"设计文档已创建: {output_file_path}")
        self._log(phase, f"Markdown 审阅文件已创建: {markdown_file}")
    ...
    def write_plan(self, design_doc_path: str, output_file: Optional[str] = None) -> str:
        ...
        # 保存实施计划
        if output_file is None:
            output_file_path = self.output_locations['plans'] / f"{phase}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        else:
            output_file_path = Path(output_file)
        ...
        self._log(phase, f"实施计划已创建: {output_file_path}")
    ...
    def execute_plan(self, plan_path: str, worktree_path: str, batch_size: int = 3) -> bool:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            # 执行 TDD 循环
            if not self.execute_tdd_cycle(task, Path(worktree_path)):
    ...
    def execute_tdd_cycle(self, task: Dict[str, Any], worktree_path: Path) -> bool:
        """
    ...
    def _create_brainstorming_markdown(self, design_doc: Dict, markdown_file: Path):
        """创建头脑风暴的 Markdown 文件用于审阅"""
        content = f"# {design_doc.get('requirement', '头脑风暴')}\n\n"
        content += "## 项目基本信息\n"
        content += f"- **阶段**: {design_doc.get('phase', '')}\n"
        content += f"- **时间戳**: {design_doc.get('timestamp', '')}\n"
        content += f"- **项目类型**: {design_doc.get('project_type', '')}\n"
        content += f"- **项目路径**: {design_doc.get('project_root', '')}\n"
        content += f"- **需求**: {design_doc.get('requirement', '')}\n"
        content += f"- **状态**: {design_doc.get('status', '')}\n\n"
        
        # 澄清问题
        questions = design_doc.get('clarifying_questions', [])
        if questions:
            content += "## 澄清问题\n\n"
        ...
        # 替代方案
        alternatives = design_doc.get('alternatives', [])
        if alternatives:
            content += "## 替代方案\n\n"
            for i, alt in enumerate(alternatives, 1):
                content += f"### {i}. {alt.get('name', '未命名方案')}\n"
                content += f"- **描述**: {alt.get('description', '')}\n"
                content += "- **优点**:\n"
        ...
        # 推荐设计
        recommended = design_doc.get('recommended_design', {})
        if recommended:
            content += "## 推荐设计\n\n"
            content += f"### {recommended.get('name', '未命名推荐')}\n"
            content += f"- **理由**: {recommended.get('rationale', '')}\n\n"
        
        # 实施计划概述
        plan_outline = design_doc.get('implementation_plan_outline', {})
        if plan_outline:
            content += "## 实施计划概述\n\n"
            
            core_components = plan_outline.get('core_components', [])
            if core_components:
                content += "### 核心组件\n"
            ...
            tech_arch = plan_outline.get('technical_architecture', [])
            if tech_arch:
                content += "### 技术架构\n"
                for arch in tech_arch:
                    content += f"- {arch}\n"
                content += "\n"
    ...
    def finish_development_branch(self, worktree_path: str, feature_name: str) -> bool:
        ...
        try:
            self._run_test_baseline(Path(worktree_path))
        except Exception as e:
