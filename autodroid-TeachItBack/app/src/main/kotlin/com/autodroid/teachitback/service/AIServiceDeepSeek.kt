...
/**
 * DeepSeek AI服务实现
 * 支持基础对话、长文本、代码生成、数学计算
 * 继承BaseAIServiceImpl，只重写支持的功能
 */
class AIServiceDeepSeek(
    private val context: android.content.Context
) : BaseAIServiceImpl() {

    override val config: AIServiceConfig = AIServiceConfig.DeepSeekConfig()
    ...
    override suspend fun sendMessage(message: MessageEntity, context: String): AIServiceResponse {
        ...
        try {
            ...
            val content = chatResponse.choices.firstOrNull()?.message?.content
                ?: throw IOException("Empty response from DeepSeek")
            ...
            throw IOException("DeepSeek API调用失败: ${e.message}", e)
        }
    }
    ...
    override suspend fun analyzeLearningProgress(conversationHistory: List<MessageEntity>): ProgressAnalysis {
        ...
        try {
            ...
            val analysis = gson.fromJson(response.content, DeepSeekAnalysisResponse::class.java)
    ...
    override suspend fun evaluateAnswer(userAnswer: MessageEntity, correctAnswer: String): AnswerEvaluation {
        ...
        try {
            ...
            val evaluation = gson.fromJson(response.content, DeepSeekEvaluationResponse::class.java)
    ...
    override suspend fun extractKeyConcepts(content: String): List<Concept> {
        ...
        try {
            ...
            val concepts = gson.fromJson(response.content, Array<DeepSeekConcept>::class.java)
    ...
    data class DeepSeekAnalysisResponse(
    ...
    data class DeepSeekEvaluationResponse(
