...
class UIEventProcessor(private val context: Context) {
    ...
    fun processClickEvent(
        ...
        // 检查是否包含敏感信息
        if (ruleEngine.containsSensitiveInfo(elementInfo.text) ||
            ruleEngine.containsSensitiveInfo(elementInfo.hint) ||
            ruleEngine.containsSensitiveInfo(elementInfo.contentDesc)) {
            // 对敏感信息进行脱敏处理
            val redactedElementInfo = elementInfo.copy(
                text = ruleEngine.redactSensitiveInfo(elementInfo.text),
                hint = ruleEngine.redactSensitiveInfo(elementInfo.hint),
                contentDesc = ruleEngine.redactSensitiveInfo(elementInfo.contentDesc)
            )
            ...
            return UIEvent(
                ...
                eventType = "CLICK",
                elementId = redactedElementInfo.id,
                elementType = redactedElementInfo.type,
                elementText = redactedElementInfo.text,
                elementHint = redactedElementInfo.hint,
                elementContentDesc = redactedElementInfo.contentDesc,
    ...
    fun processInputEvent(
        ...
        // 检查输入值是否为敏感信息
        if (ruleEngine.containsSensitiveInfo(inputValue)) {
            // 对敏感输入进行脱敏处理
            val redactedInputValue = ruleEngine.redactSensitiveInfo(inputValue)
            val redactedElementInfo = elementInfo.copy(
                text = ruleEngine.redactSensitiveInfo(elementInfo.text),
                hint = ruleEngine.redactSensitiveInfo(elementInfo.hint),
                contentDesc = ruleEngine.redactSensitiveInfo(elementInfo.contentDesc)
            ...
            // 更新元素特征（脱敏后）
            updateElementFeature(source, packageName, activityName, redactedInputValue)
    ...
    fun processSelectionEvent(
        ...
        // 检查选择值是否为敏感信息
        if (ruleEngine.containsSensitiveInfo(selectedValue)) {
            // 对敏感选择值进行脱敏处理
            val redactedSelectedValue = ruleEngine.redactSensitiveInfo(selectedValue)
            val redactedElementInfo = elementInfo.copy(
                text = ruleEngine.redactSensitiveInfo(elementInfo.text),
                hint = ruleEngine.redactSensitiveInfo(elementInfo.hint),
                contentDesc = ruleEngine.redactSensitiveInfo(elementInfo.contentDesc)
            ...
            // 更新元素特征（脱敏后）
            updateElementFeature(source, packageName, activityName, redactedSelectedValue)
            ...
            return UIEvent(
                ...
                elementBounds = getBoundsString(source),
                inputValue = null,
                selectedValue = redactedSelectedValue,
                parentHierarchy = extractParentHierarchy(source),
                siblingInfo = extractSiblingInfo(source),
