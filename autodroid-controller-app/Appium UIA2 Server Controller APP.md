...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **方案A：自研轻量Controller APP** | 开发一个Android应用，通过HTTP从公司服务器拉取测试任务（JSON格式），并直接转发给本地的 `localhost:8200` (UIA2 Server) 执行。 | **无障碍服务** 或 **ADB授权的高权限** | 1. 安装自研Controller APP。<br>2. **（可选但推荐）** 通过USB连接，使用 `adb shell pm grant` 命令授予APP修改系统设置等权限，以实现最稳定控制。 | 1. **部署极简**：员工家庭只需保持手机在线。<br>2. **纯手机端**：无需其他硬件。<br>3. **架构清晰**：Controller作为中枢，易于维护。 | 1. 若仅用无障碍服务，需用户手动开启，且权限可能被系统回收。<br>2. 若用ADB授权，需技术人员对每台手机进行一次USB初始连接，但一劳永逸。 |
...
2.  **第二步：开发Controller APP (核心)**
    ...
    *   **功能2 - 执行**：将任务转换为标准的WebDriver API请求，使用HTTP客户端（如OkHttp）发送给 `http://127.0.0.1:8200`。
    *   **功能3 - 上报**：将测试执行结果（成功/失败、日志）回传给服务器。
...
这是与UIA2 Server通信的核心模块，封装了HTTP请求。
...
class WebDriverClient(
    private val baseUrl: String = "http://127.0.0.1:8200" // UIA2 Server地址
) {
    private val client = OkHttpClient.Builder()
        .connectTimeout(30, TimeUnit.SECONDS)
        .readTimeout(120, TimeUnit.SECONDS) // 测试可能较长
        .build()
...
1.  **网络权限**：确保UIA2 Server的端口(`8200`)没有被防火墙阻挡，Controller APP能访问`127.0.0.1:8200`。
2.  **会话管理**：示例中的`SessionManager`是简化版，生产环境需要更健壮的会话恢复机制。
3.  **错误处理**：考虑网络波动、UIA2 Server崩溃等情况，添加重试和状态检查。
