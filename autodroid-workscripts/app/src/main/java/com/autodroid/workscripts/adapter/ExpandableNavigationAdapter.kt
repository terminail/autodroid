...
class ExpandableNavigationAdapter(
    ...
    private fun updateFlatItems() {
        ...
        println("Starting updateFlatItems with ${appItems.size} apps")
        
        appItems.forEach { app ->
            // 添加应用项
            items.add(app)
            println("Added app item: ${app.name}, expanded: ${app.isExpanded}")
            
            // 如果应用展开，添加流程项
            if (app.isExpanded) {
                println("App ${app.name} is expanded, adding ${app.flows?.size ?: 0} flow items")
                
                // 添加流程项
                app.flows?.forEach { flow ->
                    println("Adding flow item: ${flow.name}")
                    items.add(flow)
                ...
                println("App ${app.name} is collapsed")
            }
        }
        
        println("Final flatItems size: ${items.size}")
        flatItems = items
    ...
    override fun getItemViewType(position: Int): Int {
        return when (flatItems[position]) {
            ...
            is NavigationItem.PageItem -> VIEW_TYPE_PAGE
        }
    }
    ...
    inner class AppViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(app: NavigationItem.AppItem) {
            ...
            if (!description.isNullOrEmpty()) {
                ...
                appDescriptionTextView.visibility = View.VISIBLE
            } else {
    ...
    inner class PageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(page: NavigationItem.PageItem) {
            // 设置缩进
