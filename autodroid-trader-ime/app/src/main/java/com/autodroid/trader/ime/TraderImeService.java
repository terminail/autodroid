...
public class TraderImeService extends AccessibilityService {
    ...
    private String convertAccessibilityNodeToJson(AccessibilityNodeInfo node) throws JSONException {
        ...
        // Get the original values before redaction
        String originalClassName = node.getClassName() != null ? node.getClassName().toString() : "";
        String originalText = node.getText() != null ? node.getText().toString() : "";
        String originalContentDesc = node.getContentDescription() != null ? node.getContentDescription().toString()
                : "";
        String originalResourceId = node.getViewIdResourceName() != null ? node.getViewIdResourceName() : "";
        String originalPackageName = node.getPackageName() != null ? node.getPackageName().toString() : "";

        // Redact sensitive information if needed
        String redactedClassName = containsSensitiveInfo(originalClassName) ? redactSensitiveInfo(originalClassName)
                : originalClassName;
        String redactedText = containsSensitiveInfo(originalText) ? redactSensitiveInfo(originalText) : originalText;
        String redactedContentDesc = containsSensitiveInfo(originalContentDesc)
                ? redactSensitiveInfo(originalContentDesc)
                : originalContentDesc;
        String redactedResourceId = containsSensitiveInfo(originalResourceId) ? redactSensitiveInfo(originalResourceId)
                : originalResourceId;
        String redactedPackageName = containsSensitiveInfo(originalPackageName)
                ? redactSensitiveInfo(originalPackageName)
                : originalPackageName;

        // 添加节点的基本属性 (使用与XML相同的属性名，only if they have meaningful values)
        appendJsonProperty(jsonBuilder, "class", redactedClassName);
        appendJsonProperty(jsonBuilder, "text", redactedText);
        appendJsonProperty(jsonBuilder, "content-desc", redactedContentDesc);
        appendJsonProperty(jsonBuilder, "resource-id", redactedResourceId);
        appendJsonProperty(jsonBuilder, "package", redactedPackageName);
    ...
    public boolean containsSensitiveInfo(String text) {
        ...
        if (text.contains("@") && text.contains(".")) {
            ...
            if (Pattern.compile(emailRegex).matcher(text).find()) {
                return true;
            }
        }

        // Check for phone number patterns
        String phoneRegex = "\\b(\\+?\\d{1,3}[-.]?)?\\(?\\d{3}\\)?[-.]?\\d{3}[-.]?\\d{4}\\b";
        if (Pattern.compile(phoneRegex).matcher(text).find()) {
            return true;
        }

        // Check for credit card patterns
        String ccRegex = "\\b(?:\\d{4}[-\\s]?){3}\\d{4}\\b";
        if (Pattern.compile(ccRegex).matcher(text).find()) {
            return true;
        }
     ...
     */
    public String redactSensitiveInfo(String text) {
        ...
        String emailRegex = "\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b";
        redactedText = redactPattern(redactedText, emailRegex);

        // Redact phone numbers (various formats)
        String phoneRegex = "\\b(\\+?\\d{1,3}[-.]?)?\\(?\\d{3}\\)?[-.]?\\d{3}[-.]?\\d{4}\\b";
        redactedText = redactPattern(redactedText, phoneRegex);

        // Redact credit card numbers (with spaces or dashes)
        String ccRegex = "\\b(?:\\d{4}[-\\s]?){3}\\d{4}\\b";
        redactedText = redactPattern(redactedText, ccRegex);

        // Redact SSN pattern (XXX-XX-XXXX)
        String ssnRegex = "\\b\\d{3}-\\d{2}-\\d{4}\\b";
        redactedText = redactPattern(redactedText, ssnRegex);

        return redactedText;
    }

    private String redactPattern(String text, String regex) {
        return Pattern.compile(regex).matcher(text).replaceAll(match -> {
            String matchedText = match.group();
            if (matchedText.length() > 4) {
                return matchedText.substring(0, 2) +
                        "*".repeat(Math.max(0, matchedText.length() - 4)) +
                        matchedText.substring(Math.max(0, matchedText.length() - 2));
            } else {
                return "*".repeat(Math.max(0, matchedText.length()));
            }
        });
    }
}