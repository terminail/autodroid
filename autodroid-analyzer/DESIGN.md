...
#### 1. **前端设备卡片组件**
设备列表页面采用卡片式布局，按最后连接时间倒序排列，每张卡片包含以下信息：
...
- **操作功能**：
  - WiFi调试开通指导（针对USB连接的老款安卓手机）
  - 连接状态刷新按钮
  - 设备详情查看链接

#### 2. **WiFi调试指导功能**
根据设备Android版本提供不同的WiFi调试开通指导：

- **Android 11及以上版本**：
  - 指导用户通过开发者选项中的"无线调试"功能
  - 提供配对码获取和连接步骤说明

- **Android 9-10版本**：
  - 指导用户通过ADB命令开启WiFi调试
  - 提供`adb tcpip 5555`和`adb connect`命令
...
##### 自动设备状态更新
- **独立监控进程**：启动独立的后台进程，定期执行`adb devices`命令
- **实时状态同步**：检测到设备连接状态变化时，自动更新数据库中的设备信息
- **智能刷新策略**：
  - 设备连接时：立即获取详细设备信息并更新数据库
  - 设备断开时：标记为断开状态，保留历史记录
  - 定期巡检：每30秒检查一次设备连接状态

##### 数据库缓存机制
- **设备信息缓存**：将ADB获取的设备信息缓存到数据库，提高前端访问速度
- **连接历史记录**：记录设备的连接和断开时间，支持连接历史查询
- **状态变化通知**：当设备状态发生变化时，通过WebSocket推送给前端
...
##### WiFi调试指导API
```python
@router.get("/{device_id}/wifi-debug-guide")
async def get_wifi_debug_guide(device_id: str):
    """获取WiFi调试开通指导"""
    # 根据设备Android版本返回相应的指导信息
```

#### 5. **前端交互设计**

##### 卡片布局
- **响应式设计**：支持桌面端、平板和移动端自适应
- **卡片动画**：添加悬停效果和状态变化动画
- **批量操作**：支持多选设备进行批量操作

##### 实时更新
- **WebSocket连接**：建立WebSocket连接接收设备状态变化通知
- **自动刷新**：每30秒自动刷新设备列表
- **手动刷新**：提供手动刷新按钮，立即获取最新设备状态

##### 用户交互
- **点击展开**：点击卡片展开显示更多设备信息
- **状态指示**：使用颜色编码表示不同的连接状态
- **操作反馈**：用户操作后立即显示反馈信息
