...
import com.autodroid.manager.service.WeUPnPImplementation
...
/**
 * Test for WeUPnPImplementation mDNS discovery functionality
 */
@RunWith(AndroidJUnit4::class)
class WeUPnPImplementationTest {
    
    private lateinit var context: Context
    private lateinit var weUPnPImplementation: WeUPnPImplementation
    ...
    fun setUp() {
        ...
        weUPnPImplementation = WeUPnPImplementation(context)
    ...
    @Test
    fun testWeUPnPDiscovery() {
        // Check if WeUPnPImplementation is available
        if (!weUPnPImplementation.isAvailable()) {
            println("WeUPnPImplementation not available on this device - 实现不可用")
            return  // 实现不可用，测试通过
        }
        
        println("Starting WeUPnPImplementation discovery test...")
        
        // Start discovery
        weUPnPImplementation.startDiscovery(object : com.autodroid.manager.service.MdnsImplementation.Callback {
            override fun onServiceFound(serverInfo: com.autodroid.manager.model.ServerInfo) {
                discoveredServiceName = serverInfo.serviceName
                discoveredServiceHost = serverInfo.host
                discoveredServicePort = serverInfo.port
            ...
            override fun onDiscoveryFailed() {
                discoveryFailed = true
                println("WeUPnPImplementation discovery failed")
                countDownLatch.countDown()
            }
        })
        ...
        // Stop discovery
        weUPnPImplementation.stopDiscovery()
        
        // Verify results - 如果发现未启动，说明实现不可用，测试通过
        if (!discoveryStarted) {
            println("WeUPnPImplementation discovery not started - 实现不可用")
            return  // 实现不可用，测试通过
        }
        
        if (discoveryFailed) {
            println("WeUPnPImplementation discovery failed - this may be expected on some devices")
        } else if (discoveryCompleted) {
            if (discoveredServiceName != null && discoveredServiceHost != null && discoveredServicePort != -1) {
                println("✓ WeUPnPImplementation successfully discovered service: $discoveredServiceName at $discoveredServiceHost:$discoveredServicePort")
                ...
                println("WeUPnPImplementation discovery completed but no service found")
            }
        } else {
            println("WeUPnPImplementation discovery timed out after 30 seconds")
        }
        
        println("WeUPnPImplementation test completed")
    }
}