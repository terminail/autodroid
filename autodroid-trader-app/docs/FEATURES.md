...
2. 如果wifi功能已开启，首先尝试连接数据库中已保存的服务器：
    ...
    2. 依次尝试访问每个服务器的API端点。
    3. 如果找到可用的服务器，直接连接并更新服务器信息，停止扫描。
    4. 如果所有已保存的服务器都不可用，则启动局域网扫描。
3. 启动局域网扫描，扫描局域网内的服务器。具体实现如下：
    1. 获取当前连接的wifi网络SSID和IP前缀（如192.168.1）。
    2. 根据用户设置的IP范围（如30~80）和端口范围（如8000-8080）扫描局域网内的IP地址。
    3. 对每个IP地址，发送一个ICMP请求包（也称为ping请求）。
    4. 如果收到ICMP回复包（也称为ping回复），则认为该IP地址对应的设备可能是一个服务器。
4. 根据扫描到的可能服务器的IP地址，筛选出可以连接的服务器。具体实现如下：
    1. 对每个可能的服务器IP地址，从用户设置的起始端口开始直至结束端口，访问api：`http://<IP地址>:<端口>/api/server`。
...
5. 在扫描过程中，实时显示扫描进度和结果。
    ...
    5. IP范围输入优化：用户只需输入IP地址的末尾部分（如30~80），系统会自动获取当前WiFi的IP前缀（如192.168.1）并显示在标题中，确保扫描的IP地址与当前WiFi网络在同一子网。
...
- 支持交易计划状态管理（如批准、否决），采用类似微信多选消息的交互模式：
  - 长按交易计划项目进入多选模式
  - 多选模式下，只有状态为 PENDING、APPROVED、REJECTED 的项目显示 radio 按钮
  - 支持打勾批准或留空否决，可在批准和否决状态之间切换
  - 每次点击立即同步服务器及本地数据库
