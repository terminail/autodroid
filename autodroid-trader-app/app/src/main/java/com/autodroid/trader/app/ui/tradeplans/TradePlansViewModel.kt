...
class TradePlansViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun initialize() {
        ...
        tradePlanManager.getTradePlansByStatusAndSync(com.autodroid.trader.app.network.TradePlanStatus.ALL.value)
    ...
    private fun calculateTradePlanSummary(tradePlans: List<TradePlanEntity>): TradePlanSummary {
        ...
        return TradePlanSummary(
            pendingCount = tradePlans.count { it.status?.equals(com.autodroid.trader.app.network.TradePlanStatus.PENDING.value, ignoreCase = true) == true },
            approvedCount = tradePlans.count { it.status?.equals(com.autodroid.trader.app.network.TradePlanStatus.APPROVED.value, ignoreCase = true) == true },
            rejectedCount = tradePlans.count { it.status?.equals(com.autodroid.trader.app.network.TradePlanStatus.REJECTED.value, ignoreCase = true) == true },
            executedSuccessCount = tradePlans.count { it.status?.equals(com.autodroid.trader.app.network.TradePlanStatus.COMPLETED.value, ignoreCase = true) == true },
            executedFailedCount = tradePlans.count { it.status?.equals(com.autodroid.trader.app.network.TradePlanStatus.FAILED.value, ignoreCase = true) == true }
        )
    }
    ...
    fun refresh() {
        viewModelScope.launch {
            try {
                tradePlanManager.getTradePlansByStatus(com.autodroid.trader.app.network.TradePlanStatus.ALL.value)
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            try {
                val nextStatus = when (currentStatus?.uppercase()) {
                    "PENDING" -> com.autodroid.trader.app.network.TradePlanStatus.APPROVED.value
                    "APPROVED" -> com.autodroid.trader.app.network.TradePlanStatus.REJECTED.value
                    "REJECTED" -> com.autodroid.trader.app.network.TradePlanStatus.PENDING.value
                    else -> com.autodroid.trader.app.network.TradePlanStatus.PENDING.value
                }
