...
import com.autodroid.trader.app.network.ServerInfoResponse
import com.autodroid.trader.app.AppViewModel
import com.autodroid.trader.app.managers.ServerManager

import com.autodroid.trader.app.data.repository.ServerRepository
...
class ItemServerManager(
    ...
    private fun setupServerManagerObservers() {
        ...
        serverManager.scanState.observe(lifecycleOwner) { state ->
            when (state) {
                com.autodroid.trader.app.managers.ScanState.IDLE -> {
                    updateItem(
                        serverStatus = "IDLE"
                    )
                }
                com.autodroid.trader.app.managers.ScanState.SCANNING -> {
                    updateItem(
                        serverStatus = "SCANNING"
                    )
                }
                com.autodroid.trader.app.managers.ScanState.PAUSED -> {
                    updateItem(
                        serverStatus = "PAUSED"
                    )
                }
                com.autodroid.trader.app.managers.ScanState.COMPLETED -> {
                    updateItem(
                        serverStatus = "IDLE"
                    )
                }
    ...
    private fun startQRCodeScanner() {
        val intent = Intent(context, com.autodroid.trader.app.ui.qrcode.QrCodeScannerActivity::class.java)
    ...
    fun handleAutoScanClick() {
        val currentState = serverManager.scanState.value ?: com.autodroid.trader.app.managers.ScanState.IDLE
